...
def missings(df):
    ...
    df['grade'] = imputed['grade'].round()
    df['stage'] = imputed['stage'].round()
    df['lymph_nodes_positive'] = imputed['lymph_nodes_positive'].round()
    df['CELLULARITY'] = imputed['CELLULARITY'].round()
    return df
